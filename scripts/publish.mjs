...
`);
  ...
  const dmgFilename = `ResoStage-${version}-mac-${process.arch}.dmg`;
  const dmg = join(publishDir, dmgFilename);
  rmSync(dmg, { force: true });
  const dmgStage = join(stage, "dmg");
  mkdirSync(dmgStage, { recursive: true });
  cpSync(pkg, join(dmgStage, pkgFilename));
...
function publishWindows() {
  ...
  const archInstallMode = isArm ? "arm64" : "x64compatible";
  const vcRedistFile = isArm ? "vc_redist.arm64.exe" : "vc_redist.x64.exe";
  const vcRegKey = isArm ? "arm64" : "x64";
...
Source: "${vcRedistFile}"; DestDir: "{tmp}"; Flags: deleteafterinstall skipifsourcedoesntexist
...
[Run]
Filename: "{tmp}\\${vcRedistFile}"; Parameters: "/quiet /norestart"; \\
...
begin
  Result := True;
  if RegQueryDWordValue(HKLM, 'SOFTWARE\\Microsoft\\VisualStudio\\14.0\\VC\\Runtimes\\${vcRegKey}',
...
`);
    ...
    run("appimagetool", [appdir, join(publishDir, appImageName)], {
      env: { ...process.env, ARCH: linuxArch },
    });
    ok(`AppImage: ${join(publishDir, appImageName)}`);
  }
