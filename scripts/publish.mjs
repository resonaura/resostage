...
function publishMac() {
  ...
  // All staging + outputs go under PLATFORM_DIST_DIR (build/mac/arm64/)
  const stage = join(PLATFORM_DIST_DIR, "stage");
  rmSync(stage, { recursive: true, force: true });
  mkdirSync(join(stage, "root", "Applications"), { recursive: true });
  mkdirSync(join(stage, "scripts"), { recursive: true });
  mkdirSync(PLATFORM_DIST_DIR, { recursive: true });
...
`);
  ...
  const component = join(PLATFORM_DIST_DIR, "ResoStage-component.pkg");
  log("pkgbuild...");
  run("pkgbuild", [
...
`);
  ...
  const pkg = join(PLATFORM_DIST_DIR, `ResoStage-${version}.pkg`);
  log("productbuild...");
  run("productbuild", [
    "--distribution", distXml,
    "--package-path", PLATFORM_DIST_DIR,
    pkg,
  ]);
  ...
  const dmg = join(PLATFORM_DIST_DIR, `ResoStage-${version}.dmg`);
  ...
  ok(`pkg: ${pkg}`);
  ok(`dmg: ${dmg}`);
  ok(`notes: ${join(PLATFORM_DIST_DIR, "FIRST-RUN.txt")}`);
}
...
function publishWindows() {
  ...
  const iss = join(PLATFORM_DIST_DIR, "resostage.iss");
...
`);
  ...
  if (have("iscc")) {
    log("iscc...");
    run("iscc", [iss], { cwd: PLATFORM_DIST_DIR });
    ok(`installer: ${join(PLATFORM_DIST_DIR, `ResoStage-${version}-Setup.exe`)}`);
  } else {
...
`;
  ...
  if (have("appimagetool")) {
    const appdir = join(PLATFORM_DIST_DIR, "ResoStage.AppDir");
...
`);
    ...
    log("appimagetool...");
    run("appimagetool", [appdir, join(PLATFORM_DIST_DIR, `ResoStage-${version}-x86_64.AppImage`)]);
    ok(`AppImage: ${join(PLATFORM_DIST_DIR, `ResoStage-${version}-x86_64.AppImage`)}`);
    return;
  }
  ...
  const tar = join(PLATFORM_DIST_DIR, `ResoStage-${version}-linux-${process.arch}.tar.gz`);
  log("tar (appimagetool not found)...");
  run("tar", ["-czf", tar, "-C", payload, "."]);
  writeFileSync(join(PLATFORM_DIST_DIR, "LINUX-DEPS.txt"), `ResoStage needs, at runtime:
