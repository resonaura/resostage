...
// scripts/migrate.mjs applies to old projects (gain past unity clamps to
...
// the CURRENT on-disk shape -- older formats are converted out-of-band by
// scripts/migrate.mjs, never in here.
...
// ProjectLoader::reparseProject() can reject an outdated file BEFORE running
// the strict parser -- otherwise an old-shape project would surface as a pile
// of parse errors instead of "run pnpm migrate".
...
// a 0-based physical start channel + width. The single source of truth for
// the id scheme -- MixGraph and the routing setters both go through it.
